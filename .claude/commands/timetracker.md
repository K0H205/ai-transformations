...
Outlook カレンダーの当日予定を取得し、Time Tracker へ工数を自動入力する。

**フロー**: Outlook 予定取得 → プロジェクトマッピング → ユーザー確認 → Playwright CLI で入力
...
### Step 1: 当日の Outlook 予定を取得

ms-365-mcp-server を使用して当日の予定一覧を取得する。

```
mcp__ms365__get_events を呼び出し、
start: 今日の 00:00:00、end: 今日の 23:59:59 の範囲で予定を取得してください。
```
...
5. ユーザーが「はい」と回答した場合のみ Step 4 に進む
6. ユーザーが修正を求めた場合は内容を修正して再確認する

---

### Step 4: Playwright CLI で Time Tracker に入力

ユーザー承認後に以下を実行する。

#### 4-1. 入力スクリプトを生成

Step 2 のデータを使って `/tmp/timetracker-entry.js` を作成する:
...
// Step 2 で確定したエントリデータ
const entries = [
  { project: '社内MTG',  task: 'ミーティング',   hours: 0.5, date: '2026-02-21' },
  { project: 'ProjectA', task: '設計・レビュー', hours: 2.0, date: '2026-02-21' },
  { project: 'ProjectA', task: '開発',           hours: 4.0, date: '2026-02-21' },
  // 実際のエントリに置き換える
];
...
| 項目 | 内容 |
|------|------|
| **入力前の確認** | Step 3 のユーザー承認を必ずとること。省略・スキップ禁止 |
