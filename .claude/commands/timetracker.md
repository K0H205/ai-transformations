...
まず `.claude/skills/playwright-cli/SKILL.md` を Read ツールで読み込み、Playwright のテンプレートとリファレンスを把握すること。
...
**フロー**: 日付ヒアリング → Outlook 予定取得 → プロジェクトマッピング → ユーザー確認 → Playwright で入力
...
**以下をユーザーに提示し、明示的な承認を得ること。承認前に Playwright を実行してはいけない。**
...
### Step 5: Playwright で Time Tracker に入力

ユーザー承認後、`playwright-cli` Skill の**ループ入力テンプレート**を使用してスクリプトを生成・実行する。

#### 5-1. 入力スクリプトを生成

playwright-cli Skill のループ入力テンプレートをベースに `/tmp/timetracker-entry.js` を作成する。

**entries データ**: Step 3 の確定データを以下の形式で埋める（`date` には Step 1 の `TARGET_DATE` を使用）:
...
**フォーム操作部分**（ループ内）: Time Tracker の UI に合わせてセレクターを調整すること:
...
> **TARGET_URL** には「Time Tracker URL」セクションの URL を使用する。
> エラー時のスクリーンショットは `/tmp/timetracker-error.png` に保存する。
...
- **成功時**: `✅ 全エントリの入力が完了しました` が表示される。ユーザーに完了を報告する。
