...
- `pip install -r requirements.txt` で `github-copilot-sdk` を導入していること

環境変数:
- COPILOT_MODEL: Copilot セッションで使うモデルID（省略時は Copilot CLI のデフォルト）
"""
...
async def _run_copilot_search(query: str, cwd: str) -> str:
    ...
    await client.start()
    try:
        # モデルIDの体系は Copilot CLI 側に依存するため、COPILOT_MODEL 環境変数が
        # 指定されたときだけ渡し、未指定なら CLI のデフォルトモデルに任せる。
        session_config: dict = {"cwd": os.path.abspath(cwd)}
        if os.environ.get("COPILOT_MODEL"):
            session_config["model"] = os.environ["COPILOT_MODEL"]
